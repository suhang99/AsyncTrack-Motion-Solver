...
auto main(int argc, char** argv) -> int {
  ...
  std::vector<motion_solver::Scalar> pixel_noise_selection(11);
  for (auto i = 0; i < 11; i++) {
    pixel_noise_selection[i] = i * 0.5;
  }
  ...
  output_file << "#landmark, #measurement, pixel level, mean error" << std::endl;
  ...
  for (auto n_landmark : num_landmarks) {
    ...
    for (auto n_measurement : num_measurement) {
      ...
      for (auto pixel_noise : pixel_noise_selection) {
        config.noise.pixel_level = pixel_noise;
